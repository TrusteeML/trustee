...
class TrustReport:
    ...
    def _prepare_data(self):
        ...
        if self.feature_names is not None:
            ...
            self.feature_names = (
                list(self.X_train.columns) if isinstance(self.X_train, pd.DataFrame) else self.feature_names
            )

    def _fit_and_explain(self, dagger_ccp_alpha=0.0, dagger_max_leaf_nodes=None, dagger_max_depth=None):
        """
        ...
        if self.skip_retrain:
            ...
            if isinstance(self.blackbox, TabularPredictor):
                ...
                training_data = self.X_train.assign(**args)
                blackbox_copy.fit(training_data)
            else:
                blackbox_copy.fit(self.X_train, self.y_train)

        y_pred = getattr(blackbox_copy, self.predict_method_name)(self.X_test)
        ...
        dt_y_pred = dt.predict(self.X_test)
    ...
    def _collect_first_dt(self):
        ...
        self.first_dt_samples_by_level = np.zeros(self.first_dt.get_depth())
        self.first_dt_nodes_by_level = np.zeros(self.first_dt.get_depth()).astype(int)
    ...
    def _collect_ccp_prunning(self):
        ...
        self.ccp_iter = []
        for quantil in np.linspace(0, 1, 10, endpoint=False):
    ...
    def _collect_features_iter_removal(self):
        ...
        while it < self.max_iter and n_features_removed < self.bb_n_input_features:
            # remove most significant feature
            if isinstance(self.X_train, pd.DataFrame):
                self.X_train.iloc[:, top_feature_to_remove] = 0
                self.X_test.iloc[:, top_feature_to_remove] = 0
            elif isinstance(self.X_train, torch.Tensor):
                self.X_train[:, top_feature_to_remove] = torch.zeros(len(self.X_train))
                self.X_test[:, top_feature_to_remove] = torch.zeros(len(self.X_test))
            else:
                self.X_train[:, top_feature_to_remove] = np.zeros(len(self.X_train))
                self.X_test[:, top_feature_to_remove] = np.zeros(len(self.X_test))

            n_features_removed += 1

            dt, y_pred, dt_y_pred = self._fit_and_explain()
    ...
    def __str__(self):
        ...
        self.report = PrettyTable(title="Classification Trust Report", header=False)
        ...
        for it in self.max_leaves_iter:
            joined_similarity = "\n".join([f"  {sim:.3f}" for sim in it["similarity_vec"]])
        ...
        if not self.skip_retrain:
            ...
            for it in self.whitebox_iter:
                iter_performance.add_row(
                    [
                        it["it"],
                        self.feature_names[it["feature_removed"]] if self.feature_names else it["feature_removed"],
    ...
    def save(self, output_dir):
        ...
        if output_dir:
            ...
            with open(f"{report_output_dir}/trust_report.txt", "w", encoding="utf-8") as f:
                f.write(f"\n{self.report}")
            ...
            for idx, it in enumerate(self.ccp_iter):
                dot_data = tree.export_graphviz(
                    it["dt"],
                    class_names=self.class_names,
                    feature_names=self.feature_names,
            ...
            for idx, it in enumerate(self.max_depth_iter):
                dot_data = tree.export_graphviz(
                    it["dt"],
                    class_names=self.class_names,
                    feature_names=self.feature_names,
                ...
                graph = graphviz.Source(dot_data)
                graph.render(f"{prunning_output_dir}/max_depth_dt_{idx}_{it['dt'].tree_.node_count}")

            for idx, it in enumerate(self.max_leaves_iter):
                dot_data = tree.export_graphviz(
                    it["dt"],
                    class_names=self.class_names,
                    feature_names=self.feature_names,
                ...
                graph = graphviz.Source(dot_data)
                graph.render(f"{prunning_output_dir}/max_leaves_dt_{idx}_{it['dt'].tree_.node_count}")
