...
def dagger_test_dnn(dataset_meta, validate_dataset_path="", resampler=None, max_leaves=None, ccp_alpha=0.0, num_samples=2000, as_df=False):
    """ Test using Reinforcement Learning to extract Decision Tree from a generic Blackbox model """
    logger = log.Logger(
        "{}/res/log/{}/dagger_test_dnn_{}_{}.log".format(rootpath.detect(),
                                                         dataset_meta["name"],
                                                         "DNN",
    ...
    model_path = "../res/weights/{}_{}_{}.h5".format("DNN",
                                                     resampler.__name__ if resampler else "Raw",
                                                     dataset_meta['name'])
    ...
    try:
        ...
        blackbox.compile(optimizer="adam", metrics=['accuracy' if dataset_meta["type"] == "classification" else "mse"],
                         loss="categorical_crossentropy" if dataset_meta["type"] == "classification" else "mean_squared_error")

        logger.log("#" * 10, "Model fit", "#" * 10)
        blackbox.fit(X_train, y_train, validation_split=0.2, epochs=100,
                     verbose=2, batch_size=int(X_train.shape[0] / 100), workers=8)
    ...
    with open(RESULTS_FILE_NAME.format(rootpath.detect()), "a") as csv_file:
        ...
        csv_writer.writerow([dataset_meta["name"], len(X), "DNN", resampler.__name__ if resampler else "None",
...
def main():
    ...
    with open(RESULTS_FILE_NAME.format(rootpath.detect()), "w") as csv_file:
        ...
        csv_writer.writerow(["dataset", "dataset size", "model", "resampler",
                             "num leaves", "ccp_alpha", "blackbox f1/r2", "DT f1/r2", "fidelity"])
