...
def shap_test(dataset_meta, model=RandomForestClassifier, resampler=None, as_df=False):
    ...
    logger = log.Logger(
        "{}/res/log/shap_test_{}_{}_{}.log".format(rootpath.detect(),
                                                   model.__name__,
...
def shap_dependence_test(dataset_meta, model=RandomForestClassifier, resampler=None, as_df=False):
    ...
    for feat in range(len(feature_names)):
        inds = shap.approximate_interactions(feat, shap_values[cls], X_test)
        for i in range(3):
...
def main():
    ...
    # shap_test(CIC_IDS_2017_DATASET_META, as_df=True)
