...
def lime_test(dataset_meta, model=RandomForestClassifier, resampler=None, as_df=False):
    ...
    logger = log.Logger(
        "{}/res/log/lime_test_{}_{}_{}.log".format(rootpath.detect(),
                                                   model.__name__,
    ...
    X_test_case, y_test_case, feature_names, _, _ = dataset.read(io.StringIO(DF_TEST), metadata=dataset_meta,
    ...
    explainer = LimeTabularExplainer(X_train, feature_names=feature_names,
                                     class_names=dataset_meta['classes'], discretize_continuous=True)
    exp = explainer.explain_instance(X_test_case[0], blackbox.predict_proba,
                                     num_features=X_test_case.shape[1], labels=[8])
